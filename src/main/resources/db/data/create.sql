...
CREATE TABLE IF NOT EXISTS `qualleinc`.`purchase` (
  ...
  INDEX `fk_purchase_card_idx` (`card_id` ASC) VISIBLE,
  INDEX `fk_purchase_metadata_idx` (`metadata_id` ASC) VISIBLE,
  ...
  CONSTRAINT `fk_purchase_product`
    ...
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_purchase_card`
    FOREIGN KEY (`card_id`)
    REFERENCES `qualleinc`.`card` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_purchase_metadata`
    FOREIGN KEY (`metadata_id`)
    REFERENCES `qualleinc`.`metadata` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
